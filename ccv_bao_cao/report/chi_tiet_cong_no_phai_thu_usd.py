...
class chi_tiet_cong_no_phai_thu_usd(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        # Đầu đề
        sheet.merge_range("A4:S4","CHI TIẾT CÔNG NỢ PHẢI THU",format1)
        sheet.set_row(4, 35)
        sheet.set_row(5, 20)
        sheet.merge_range("A5:S5","Tài khoản: %s; Loại tiền: USD; Khách hàng: %s; Từ ngày %s đến ngày %s" % (account, partner, date_start,date_end),format21)
        ...
        sheet.merge_range(f"L{prod_row}:S{prod_row}","",  footer_font_size_title)
