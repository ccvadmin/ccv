...
async function changeMarkerPosition(marker) {
	vehicleLocation = await getLocation()
		.then((location) => location)
		.catch((error) => {
			console.error(error);
		});
	var newLatLng = new google.maps.LatLng(vehicleLocation.lat, vehicleLocation.lng);
...
async function initMap() {
	...
	await getPosition()
	if (role == 'driver') {
		setInterval(postPosition, 10000);
	}
	...
	// Khởi tạo bản đồ
	let center = checkLocation(vehicleLocation) ? vehicleLocation : destinationLocation
	...
	calculateRoute(vehicleLocation, destinationLocation);
}
