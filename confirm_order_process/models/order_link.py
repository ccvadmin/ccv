                ...
                if record.sale_order_id.id == val.get('sale_order_id', False):
                    if not val.get('url', False):
                        if record.type == 'order':
                            record.url = self._generate_order_url(record.sale_order_id)
                        elif record.type == 'delivery':
                            record.url = self._generate_delivery_url(record.sale_order_id)
                        elif record.type == 'image':
                            record.url = self._generate_image_url(record.sale_order_id)
                    else:
